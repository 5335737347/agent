...
def make_read_file_tool(workspace: Workspace, *, max_lines: int = 200) -> Tool:
    """Create a bounded UTF-8 file-reading tool for a workspace.

    Args:
        workspace: Workspace that constrains accessible paths.
        max_lines: Maximum number of lines returned by one call.

    Returns:
        A configured ``read_file`` tool.
    ...
    def read_file(
        ...
        Returns:
            Numbered file content or a descriptive error.
        """
        if start_line < 1:
        ...
        requested_end = (
            end_line if end_line is not None else start_line + max_lines - 1
        )
        ...
        try:
            with target.open(encoding="utf-8") as source:
                raw_lines = list(islice(source, start_line - 1, requested_end + 1))
        ...
        if has_more:
            content += (
                f"\n[Output truncated. Continue with start_line = "
                f"{requested_end + 1}.]"
            )
        return content
